...
- 最前面表示とフルスクリーン表示に対応。
- タスクバー点滅や Windows 通知に反応したウィンドウを、サムネイルの赤枠/オレンジ枠で気づけます。
- 元ウィンドウが現在アクティブなセルを、白枠で見分けられます。
...
- 点滅中や通知発生ウィンドウを見逃しにくい赤枠/オレンジ枠通知。
- 現在アクティブな元ウィンドウが分かる白枠表示。
- 監視専用に使いやすいフルスクリーン表示。
- 元ウィンドウが閉じたときの自動クリーンアップ。
- 登録したアプリを自動追加できます。WindowThumbWall の実行中に作成されたウィンドウは、アプリごとの検出順で追加され、登録アプリ同士の順番は設定どおりに保たれます。

## 自動追加時の順番

自動追加は Windows のタスクバーに近い安定した順番を使いますが、タスクバーの正確な並びを読み取る機能ではありません。起動時点ですでに開いているウィンドウは、取得できるウィンドウ状態から最善の初期順を決めます。起動後に作成されたウィンドウは検出された作成順を保ちます。すでに監視しているウィンドウを自動的に並べ替えることはなく、その後に手動で行ったタスクバーの並べ替えも追跡しません。
...
5. 監視中のアプリがタスクバーで点滅したり、Windows 通知から一意に対応付けられたりすると、壁面上でも赤枠で通知されます。
6. 通知候補が複数残る場合は、その候補スロットがオレンジ枠で点滅します。ただし、同一アプリでタスクバー由来の赤点滅がある場合は、その通知のオレンジ枠は抑制されます。
7. 元ウィンドウが現在アクティブなセルには、点滅しない白枠が付きます。
8. Enter キーでフルスクリーンを切り替えます。
9. Esc キーでフルスクリーンを終了します。
10. 左下のヒントからショートカット一覧を開けます。
...
- 点滅中や通知を出した可能性が高いウィンドウを赤枠/オレンジ枠で見つけやすい。
- 今アクティブな元ウィンドウを白枠で追いやすい。
- サムネイルから元ウィンドウへすぐ戻れる。
...
Codex の「実行する」ダイアログに入力するコマンド例も、開発者ガイドにまとめています。通常実行は `dotnet run --project WindowThumbWall.csproj`、通知機能の実機確認は `.\packaging\build-and-run-msix.ps1 -Configuration Release` を使ってください。

- [開発者ガイド](docs/developer-guide.md)
- [リリース手順](docs/releasing.md)
- [不変条件](docs/invariants.md)
- [通知 attention 設計](docs/notification-attention-design.md)
